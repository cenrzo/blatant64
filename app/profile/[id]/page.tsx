    ...
    <main className="min-h-screen bg-[#F5F7FB] relative font-sans">
      ...
      <div className="pt-[104px] px-6 md:px-12 max-w-[1400px] mx-auto pb-20 relative z-10">
        <div className="flex items-center mb-10">
          <h1 className="text-3xl font-extrabold text-[#111111]">User Profile</h1>
        </div>

        <div className="flex flex-col gap-10">
          {/* Top Section: Profile Info */}
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
            <div className="md:col-span-5 lg:col-span-4 p-8 rounded-[14px] bg-white border border-[#E7ECF3] shadow-[0_4px_24px_rgba(107,122,144,0.04)] flex flex-col items-center text-center">
              <div className="relative w-28 h-28 rounded-full overflow-hidden mb-4 border border-[#E7ECF3] shadow-sm">
                {profile.profileImage ? (
                  <Image src={profile.profileImage} alt={profile.displayName || "User"} fill className="object-cover" />
                ) : (
                  <div className="w-full h-full bg-[#556457] flex items-center justify-center text-white font-bold text-3xl">
                    {(profile.displayName || "User").split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)}
                  </div>
                )}
              </div>
              
              <h2 className="text-xl font-extrabold text-[#111111] tracking-tight mb-1">
                {profile.displayName || "Anonymous User"}
              </h2>
              <p className="text-[13px] text-[#6B7A90] font-medium mb-6">
              ...
              <div className="w-full flex flex-col gap-3 text-[14px] text-[#6B7A90] mb-6">
              ...
              {isOwner && (
                <Link 
                  href="/profile/edit"
                  className="w-full mt-auto py-3 rounded-[12px] bg-white text-[#111111] font-bold text-[14px] flex items-center justify-center gap-2 hover:bg-[#F5F7FB] transition-all border border-[#E7ECF3] shadow-sm"
                >
            ...
            <div className="md:col-span-7 lg:col-span-8 h-full">
              <div className="p-8 rounded-[14px] bg-white border border-[#E7ECF3] shadow-[0_4px_24px_rgba(107,122,144,0.04)] h-full flex flex-col">
                <h3 className="font-extrabold text-[#111111] mb-4 text-xl">About Me</h3>
                <p className="text-[#6B7A90] leading-relaxed text-[15px] whitespace-pre-wrap">
          ...
          <div className="space-y-10">
            <div>
              <h2 className="text-2xl font-extrabold text-[#111111] tracking-tight mb-6">
                    ...
                    .map((venue) => {
                      ...
                      return (
                        <div key={venue.id} className="relative">
                          {isPinned && (
                            <div className="absolute top-4 right-4 z-20 px-3 py-1 rounded-full bg-[#111111] text-white text-[10px] font-bold uppercase tracking-widest shadow-lg">
                              Pinned
                            </div>
                          )}
                          <FeaturedVenueCard
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-extrabold text-[#111111] tracking-tight">Guest Reviews</h2>
                <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#F5F7FB] text-[#111111] font-bold text-sm">
              ...
              {reviews.length > 0 ? (
                <div className="space-y-6">
                  {reviews.map((review) => (
                    <div key={review.id} className="p-8 rounded-[14px] bg-white border border-[#E7ECF3] shadow-sm">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-[#F5F7FB] flex items-center justify-center overflow-hidden border border-[#E7ECF3]">
                            {review.reviewerImage ? (
                              <img src={review.reviewerImage} alt="" className="w-full h-full object-cover" />
                            ) : (
                              <div className="w-full h-full bg-[#556457] flex items-center justify-center text-white font-bold text-sm">
                                {(review.reviewerName || "User").split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2)}
                              </div>
                            )}
                          </div>
                          <div>
                            <p className="font-bold text-[#111111]">{review.reviewerName}</p>
