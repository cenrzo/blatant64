...
const HeroBanner = memo(function HeroBanner() {
  return (
    <div className="houses-hero" style={{ height: "65vh" }}>
      <Image
...
const PropertyCard = memo(function PropertyCard({ property }: { property: PropertyData }) {
  ...
  const [imageSrc, setImageSrc] = useState(
    property.image.startsWith("firestore://") ? null : property.image
  )
  const [imageLoading, setImageLoading] = useState(property.image.startsWith("firestore://"))

  useEffect(() => {
    let isMounted = true
    async function resolveImage() {
      if (property.image.startsWith("firestore://")) {
        try {
          const imageId = property.image.replace("firestore://", "")
  ...
  const finalImage = imageSrc && imageSrc.trim() !== "" ? imageSrc : "/images/venues/garden-villa.jpg"

  return (
    <Link href={`/venues/${property.id}`} className="houses-card">
      <div className="houses-card__image-wrap">
        {imageLoading ? (
          ...
          <Image
            ...
            alt={property.title}
            fill
            className="houses-card__image"
            sizes="(min-width: 1024px) 50vw, 100vw"
          />
        )}
        <div className="houses-card__image-overlay" />
        
        {property.tag === "Popular" && (
          <div className={`houses-card__tag ${isGeorgian ? "font-georgian" : ""}`}>
        ...
        >
          <Heart className="w-4 h-4 text-[#26215c]" />
        </button>
      </div>
      ...
      <div className="houses-card__body">
        ...
        <p className="houses-card__location">{property.location}</p>
        
        <div className="houses-card__meta">
          <div className="houses-card__price">
            <strong>${property.price}</strong> / night
          </div>
          <div className="houses-card__rating">
            <Star className="w-3.5 h-3.5 fill-[#26215c] text-[#26215c]" />
            <span className="houses-card__rating-value">{property.rating.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </Link>
      ...
      <div className="houses-sidebar__section">
        ...
        <div style={{ position: "relative" }}>
          ...
          >
            ...
            <option value="Birthday Centers">Birthday Centers</option>
          </select>
...
export default function HousesPage() {
  ...
  const filteredProperties = properties.filter((p) => {
    if (selectedCategory !== "all") {
      ...
      let isMatch = badge === cat
      if (cat === "houses" && ["villas", "villa", "apartments", "apartment", "house", "houses"].includes(badge)) isMatch = true
      if (cat === "spaces" && ["rooftops", "rooftop", "studios", "studio", "space", "spaces"].includes(badge)) isMatch = true
      if (cat === "birthday centers" && ["birthday centers", "birthday center", "kids venue"].includes(badge)) isMatch = true
